 ...
 *   ihpapi.h
 *
 *   Encode buffer as an Intellon VS_MDIO_COMMAND Management Message and
...
#ifndef MDIO_COMMAND_SOURCE
#define MDIO_COMMAND_SOURCE
...
size_t ihpapi_MdioCommand ( uint8_t sa [], uint8_t da [], MdioCmdInfo*pMdioCmdInfo, size_t bufferLen, uint8_t buffer [] ) 
{
	...
	offset += EncodeIntellonHeader (buffer + offset, bufferLen - offset, (VS_MDIO_COMMAND | MMTYPE_REQ));
	...
	memcpy((buffer + offset), (char *)pMdioCmdInfo, sizeof(struct MdioCmdInfo));
	offset += sizeof(struct MdioCmdInfo);

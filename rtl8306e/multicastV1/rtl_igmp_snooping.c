...
static int32 rtl_initTimerMappingTable(uint32  cpuPortNum);
...
static int32 rtl_checkMCastAddrMapping(uint32 ipAddr, const uint8* macAddr);
...
static void rtl_processPimDm(uint8* pktBuf, uint32 pktLen, uint8 pktPortMask);
 ...
 int  rtl_igmpSend(uint8 * macFrame, uint8 fwdPortMask,struct rtl_macFrameInfo macFrameInfo);
...
#endif
    uint8 multicastRouterPortMask;

    multicastRouterPortMask = rtl_getMulticastRouterPortMask();
...
#endif          
#if defined(CONFIG_RTL8306E)
                portMask.bits[0] = fwdPortMask;
                if(rtk_l2_mcastAddr_add((rtk_mac_t *)multicastMacAddress, 1, portMask ) == RT_ERR_OK)
#elif defined(CONFIG_RTL8306SDM)
...
#endif
#if defined(CONFIG_RTL8306E)
                if(rtk_l2_mcastAddr_del((rtk_mac_t *)multicastMacAddress, 1) == RT_ERR_OK)
#elif defined(CONFIG_RTL8306SDM)
    ...
    else /*here means non-aggregator entry*/
    {
        ...
        {   
            ...
            {   
...
#if defined(CONFIG_RTL8306E)
                portMask.bits[0] = fwdPortMask;                
                if (rtl8306e_l2_multicastMac_add(multicastMacAddress, TRUE, portMask.bits[0], &entryaddr) != SUCCESS)
#elif defined(CONFIG_RTL8306SDM)
                ...
                {
#ifdef IGMP_DEBUG
                    rtl_igmpGluePrintf("set multicast Mac success, and entryaddr = %d.\n", entryaddr);
#endif
                }
            }
...
#endif
                {
                    /* maybe lookup table collision, or run out of multicast entry*/
#ifdef IGMP_DEBUG
                    rtl_igmpGluePrintf("hareware failure\n");
#endif
                    return FAILED;
        ...
        {
            if(IN_LOOKUP_TABLE == groupEntry->lookupTableFlag)
            {   
#ifdef IGMP_DEBUG
  ...
  */
    ...
    {   
        ...
        {   
            ...
            {
                if((rtl_compareMacAddr(macFrame, rtl_gatewayMac) == SUCCESS) &&\
            ...
            {   
                if(rtl_checkMCastAddrMapping(ntohl(macInfo->ipBuf->destinationIp),macFrame) == SUCCESS)
                {
                    macInfo->l3Protocol = IGMP_PROTOCOL;
                }
            }
...
{   
    ...
    /*update group member timer*/
    hashIndex = groupAddress & rtl_fwdHashMask;
    if(NULL == rtl_fwdHashTable[hashIndex])   /*means new group address, create new hash entry*/
    {
    ...
    {   
        ...
        {
            hashIndex = newGroupEntry->groupAddress & rtl_fwdHashMask; 
            struct rtl_groupEntry *entryPtr = rtl_fwdHashTable[hashIndex];
            while(entryPtr != NULL)
            {
    ...
    if(groupEntry != NULL)
    {
        ...
        {
            hashIndex = groupEntry->groupAddress & rtl_fwdHashMask; 
            struct rtl_groupEntry *entryPtr = rtl_fwdHashTable[hashIndex];
            while(entryPtr != NULL)
            {
                if(((entryPtr->lookupTableFlag & IN_LOOKUP_TABLE) == 0) || ((entryPtr->lookupTableFlag & AGGREGATOR_FLAG)!=0))
        ...
        if(groupEntry != NULL)
        {
            ...
            if((groupEntry->portTimer[timerIndex] != 0)  &&  (groupEntry->portTimer[timerIndex] > rtl_multicastParas.lastMemberAgingTime))
            {
                ...
                if(TRUE == rtl_delPortMaskRevLeave)
                {
                    ...
                    groupEntry->portTimer[timerIndex] = 0;
                    rtl_set8306LookupTable(groupEntry);
...
static uint8 rtl_processIgmp(uint8* pktBuf, uint32 pktLen, uint8 pktPortMask)
{
    ...
    /* checksum error*/
    if(rtl_checksum(pktBuf, pktLen) != 0)
    {
        return 0x00;
    }
...
{ 
    ...
    if((2 == mospfHeader->version) && (MOSPF_HELLO_TYPE == mospfHeader->type))
    {
...
endLoop:
    if(0 == expiredEntry->fwdPortMask) /*means all timer have been expired*/
    {
        /* delete this multicast entry*/
        rtl_unlinkGroupEntry(expiredEntry, hashIndex);
        rtl_clearGroupEntry(expiredEntry);
...
#if defined(CONFIG_RTL8306E)
        rtk_cpu_tagPort_get(&cpuPortNum, &mode);
#elif defined(CONFIG_RTL8306SDM)
 ...
 */
int32 rtl_setIgmpSnoopingV1Parameter(struct rtl_igmpSnoopingParameter igmpSnoopingParas, uint8* pGatewayMac, uint32 gatewayIp)
{
    uint8 cpuPortNum;
    uint32 entryaddr = 0;
    ...
    if(pGatewayMac != NULL)
    {
        ...
        {
#if defined(CONFIG_RTL8306E)
            rtl8306e_l2_mac_del(rtl_gatewayMac, &entryaddr);
#elif defined(CONFIG_RTL8306SDM)
            rtl8306_deleteMacAddress(rtl_gatewayMac, &entryaddr);
#endif
        }
        
        rtl_gatewayMac[0] = pGatewayMac[0];
        rtl_gatewayMac[1] = pGatewayMac[1];
        rtl_gatewayMac[2] = pGatewayMac[2];
        rtl_gatewayMac[3] = pGatewayMac[3];
        rtl_gatewayMac[4] = pGatewayMac[4];
        rtl_gatewayMac[5] = pGatewayMac[5];
...
#ifndef RTL_IGMP_SNOOPING_TEST
    #if defined(CONFIG_RTL8306E)
        if(rtl8306e_l2_unicastMac_add(rtl_gatewayMac, 0x02, TRUE, TRUE, cpuPortNum, &entryaddr) != RT_ERR_OK)
    #elif defined(CONFIG_RTL8306SDM)
 ...
 */
void rtl_igmpSnoopingV1Receive(uint8 * pMacFrame , uint8 *pPort)
{
    ...
    memset(&macFrameInfo, 0, sizeof(macFrameInfo));
    if(NULL == pPort)
            ...
            case PIM_PROTOCOL:
                ...
                break;
                 ...
                 case MULTICAST_DATA_PACKET:
                        fwdPortMask=rtl_processData(&macFrameInfo);
...
#ifdef RTL_IGMP_SNOOPING_TEST
        pMacFrame [15] = fwdPortMask;/*set forward packet mask*/
        return;
#else
        if(fwdPortMask != 0 )
        {
  ...
  #endif
            rtl_igmpSend(pMacFrame, fwdPortMask, macFrameInfo);
        }
 #endif
    }
    else
 ...
 int  rtl_igmpSend(uint8 * macFrame, uint8 fwdPortMask,struct rtl_macFrameInfo macFrameInfo)
 {
    ...
    /*assign destination portmask through CPU tag. Please modify here when used VLAN tag and so on*/    

    macFrame[15] = fwdPortMask;/*set forward packet mask*/
    rtl_igmpGlueNicSend(macFrame, (uint32)macFrameInfo.macFrameLen);
    return OK;
 ...
 */
int32 rtl_igmpSnoopingV1Send(uint8 *pMacFrame, uint32 priorFreeBytes,  uint32 posteriorFreeSpace, uint8 **ppNewMacFrame )
{
    ...
    if(rtl_igmpStatus == ENABLE)
    {
        ...
        if(IS_CLASSD_ADDR(destIp) && (macInfo.cpuTagFlag == 0))          /*check whether destination ip is a multicast address*/
        {
            ...
            if(groupEntry != NULL)
            {
                ...
                *(*ppNewMacFrame  + 15) = (groupEntry->fwdPortMask | multicastRouterPortMask) & (~rtl_cpuPortMask);
            } 
        }
    }
 ...
 */
#ifdef RTL_CPU_HW_FWD
int32 rtl_checkGroupStatus(uint32 groupAddress, uint32 *pLutFlag, uint32 *pAggregatorFlag)
{
    struct rtl_groupEntry* groupEntry = NULL;
    uint32 hashIndex = 0; 
    ...
    else
    {
        ...
        else
        {
            ...
            if((groupEntry->lookupTableFlag & AGGREGATOR_FLAG) != 0)
            {
                *pAggregatorFlag = TRUE;
            }
            else
            {
                *pAggregatorFlag = FALSE;
            }
    ...
    if(rtl_igmpStatus == ENABLE)
    {
        ...
        for(i = 0; i < rtl_fwdHashTableSize; i++)
        {
            ...
            while(groupEntryPtr != NULL)
            {
                ...
                {
#if defined(CONFIG_RTL8306E)
                    if(rtl8306e_l2_mac_del(rtl_gatewayMac, &entryaddr) == SUCCESS)
#elif defined(CONFIG_RTL8306SDM)
                    if(rtl8306_deleteMacAddress(multicastMacAddress, &entryaddr) == SUCCESS)
#endif
                    {

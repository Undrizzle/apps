#include <assert.h>
#include "cmm_sysMonitor.h"

T_UDP_SK_INFO SK_CMM_SYSMONITOR;

int __cmm_sysmonitor_communicate(uint8_t *buf, uint32_t len)
{
	fd_set fdsr;
	int maxsock;
	struct timeval tv;
	...
	int FromAddrSize = 0;
	int rev_len = 0;
	T_UDP_SK_INFO *sk = &SK_CMM_SYSMONITOR;
	...
	if ( -1 == sendn )
	{
		fprintf(stderr, "ERROR: cmm communicate with sysMonitor sendto failed\n");
		return -1;
	}
	...
	while(1)
	{
		...
		if( ret <= 0 )
		{
			fprintf(stderr, "ERROR: cmm communicate with sysMonitor select failed\n");
			return -1;
		}
		...
		if (FD_ISSET(sk->sk, &fdsr))
		{
			...
			if ( -1 == rev_len )
			{
				fprintf(stderr, "ERROR: cmm communicate with sysMonitor recvfrom failed\n");
				return -1;
			}
			else
			{
				...
				{
					fprintf(stderr, "WARNNING: cmm communicate with sysMonitor msgType[%d != %d], [continue] !\n", 
						r->usMsgType, msgType);
					continue;
				}
				if( r->result )
				{
					fprintf(stderr, "ERROR: cmm communicate with sysMonitor result = %d\n", r->result);
				}
				return r->result;
			}			
		}
		...
		{
			fprintf(stderr, "ERROR: cmm communicate with sysMonitor FD_ISSET failed\n");
			return -1;
		}
	}
}

int cmm2sysmonitor_sysledCtrol(uint8_t event)
{
	uint32_t len = 0;
	...
	*req_data = event;
	...
	return __cmm_sysmonitor_communicate(buf, len);
}

int cmm2sysmonitor_destroy(void)
{
	T_UDP_SK_INFO *sk = &SK_CMM_SYSMONITOR;
	if( sk->sk != 0)
	{

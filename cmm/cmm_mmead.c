...
int mmead_get_Hg_Wan_Status(uint8_t ODA[], T_szHgWanStatus *wanInfo)
{
	int len = 0;
	uint8_t buf[MAX_UDP_SIZE] = {0};
	T_Msg_Header_MMEAD h;
	T_REQ_Msg_MMEAD *r = (T_REQ_Msg_MMEAD *)buf;
	bzero(wanInfo, sizeof(T_szHgWanStatus));
	bzero(buf, MAX_UDP_SIZE);

	h.M_TYPE = 0xCC08;
	h.DEV_TYPE = WEC701W_C4;
	h.MM_TYPE = MMEAD_GET_HG_WAN_STATUS;
	h.fragment = 0;
	memcpy(h.ODA, ODA, 6);

	h.LEN = sizeof(T_szHgWanStatus);

	memcpy(buf, &h, sizeof(T_Msg_Header_MMEAD));
	len = sizeof(T_Msg_Header_MMEAD) + sizeof(T_szHgWanStatus);

	if( CMM_SUCCESS == __cmm_mmead_communicate(buf, len))
	{
		memcpy(wanInfo, (void *)r->BUF, sizeof(T_szHgWanStatus));
		return CMM_SUCCESS;
	}
	return CMM_FAILED;
}

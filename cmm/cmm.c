...
int CMM_ProcessResetHg(BBLOCK_QUEUE *this)
{
	int opt_sts = CMM_SUCCESS;
	st_dbsCnu cnu;
	uint8_t bMac[6] = {0};
	...
	else if( CMM_SUCCESS != mmead_reset_Hg(bMac))
	{
		printf("\n#ERROR[05]\n");
		opt_sts = CMM_FAILED;
	}
...
int CMM_ProcessReadHgSsidStatus(BBLOCK_QUEUE *this)
{
	int opt_sts = CMM_SUCCESS;
	st_dbsCnu cnu;
	uint8_t bMac[6] = {0};
	...
	else if( CMM_SUCCESS != mmead_get_Hg_Ssid_Status(bMac, &ack_data) )
	{
		printf("\n#ERROR[05]\n");
		opt_sts = CMM_FAILED;
	}
	...
	/* 将处理信息发送给请求者 */
	CMM_ProcessAck(opt_sts, this, (T_szHgSsid *)&ack_data, sizeof(T_szHgSsid));

	return opt_sts;
}

int CMM_ProcessWriteHgSsidStatus(BBLOCK_QUEUE *this)
{
	int opt_sts = CMM_SUCCESS;
	st_dbsCnu cnu;
	uint8_t bMac[6] = {0};
	...
	else if( CMM_SUCCESS != mmead_set_Hg_Ssid_Status(bMac, req_data) )
	{
		printf("\n#ERROR[05]\n");
		opt_sts = CMM_FAILED;
	}
...
void cmmProcessManager(void)
{
	...
	while(1)
	{
		...
		switch(h->usMsgType)
		{
			...
			case CMM_RESET_HG:
			{
				opt_sts = CMM_ProcessResetHg(this);
				break;
			}
			case CMM_GET_HG_SSID_STATUS:
			{
				opt_sts = CMM_ProcessReadHgSsidStatus(this);
				break;
			}
